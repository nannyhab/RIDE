"""
Track Parameter Optimizer - WORKING VERSION
Uses manual controller instead of autopilot (more stable)
"""
...
def simple_controller(vehicle, waypoints):
    """Simple pure pursuit - NO AUTOPILOT"""
    ...
    # Target ahead
    target_idx = min(nearest_idx + 15, len(waypoints) - 1)
...
def test_one_config(world, waypoints, features, track_id, gear, friction):
    """Test ONE config - MANUAL CONTROL"""
    ...
    try:
        ...
        for step in range(2400):
            # Manual control
            throttle, steer = simple_controller(vehicle, waypoints)
    ...
    for tid in range(NUM_RANDOM_TRACKS):
        ...
        for gear in GEAR_RATIOS:
            for friction in TIRE_FRICTIONS:
                res = test_one_config(world, wps, feats, tid, gear, friction)
                if res:
                    results.append(res)
...
def main():
    ...
    print("Manual Controller (No Autopilot)")
    print("="*60)
