"""
Track Parameter Optimizer - FINAL WORKING VERSION
"""
...
# Copy RandomTrackGenerator and TrackFeatureExtractor from robust version
# (keeping them exactly the same)

from track_param_optimizer_robust import RandomTrackGenerator, TrackFeatureExtractor

def get_waypoints_safe(world):
    """Get waypoints without destroying anything"""
    map_obj = world.get_map()
    
    wp = None
    for lane_id in (-1, 1):
        for s0 in (1.0, 5.0, 10.0):
            try:
...
def test_one_config(world, waypoints, features, track_id, gear, friction):
    """Test ONE configuration on existing world"""
...
def collect_data():
    ...
    for tid in range(NUM_RANDOM_TRACKS):
        ...
        # Test all configs on THIS world
        results = []
